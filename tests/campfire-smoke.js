...
assert.ok(offer.length >= 5, 'fresh campaign can field five');
assert.ok(new Set(offer.map(e=>e.heroClass)).size >= 4, 'base classes remain represented when the fifth slot uses a duplicate');
const oldRosterCount = DS.Meta.heroRoster.length;
[2,0,3,1,4].forEach(DS.Campfire.toggleSelect);
assert.deepEqual(Array.from(DS.Campfire._selected),[2,0,3,1,4]);
DS.Campfire.toggleSelect(0);
assert.deepEqual(Array.from(DS.Campfire._selected),[2,3,1,4], 'removal closes rank gap');
DS.Campfire.toggleSelect(0);
if (offer.length > 4) {
  DS.Campfire.toggleSelect(5);
  assert.equal(DS.Campfire._selected.length,5,'cannot add a sixth hero');
}
...
assert.deepEqual(Array.from(DS.State.run.heroes,h=>h.pos),[1,2,3,4,5]);
assert.deepEqual(Array.from(DS.State.run.heroes,h=>h.maxHp),expectedHp,'preview matches current engine');
assert.equal(DS.State.run.deck.length,40);
...
DS.Meta.heroRoster = [];
DS.Campfire._buildOffer();
assert.equal(DS.Campfire._offer.length,5,'wipe still permits a complete fresh party');
